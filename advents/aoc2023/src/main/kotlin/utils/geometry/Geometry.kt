...
import com.gilpereda.aoc2022.utils.geometry.Direction.*
...
class Point private constructor(
    ...
    fun move(orientation: Orientation, steps: Int = 1): Point =
        when (orientation) {
...
data class Polygon(
    ...
    fun external(): Polygon {
        val newSides = (listOf(sides.last()) + sides).windowed(2).map { (previous, side) ->
            when (previous.orientation.turnedDirectionTo(side.orientation)) {
                RIGHT -> when (side.orientation) {
    ...
    private val clockwise: Boolean by lazy {
        (sides + sides.first()).windowed(2) { (side, next) ->
    ...
    val area: Double by lazy {
        sides.sumOf { (from, to) -> from.x.toDouble() * (to.y + 1).toDouble() - (from.y + 1).toDouble() * to.x.toDouble() }.absoluteValue / 2
    }
...
data class Line(
    ...
    val orientation: Orientation by lazy {
        ...
        else if (from.y <= to.y) SOUTH else NORTH
    }
