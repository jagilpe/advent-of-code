...
data class LongPoint3D(
    ...
    fun decomposed2D(): Pair<Double, Double> = Pair(x.toDouble(), y.toDouble())

    operator fun minus(other: LongPoint3D): LongPoint3D = copy(x = x - other.x, y = y - other.y, z = z - other.z)

    operator fun plus(other: LongPoint3D): LongPoint3D = copy(x = x + other.x, y = y + other.y, z = z + other.z)
